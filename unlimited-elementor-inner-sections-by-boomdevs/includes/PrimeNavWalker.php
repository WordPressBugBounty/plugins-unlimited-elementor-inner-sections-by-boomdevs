...
class PrimeNavWalker extends \Walker_Nav_Menu {	
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
		    ...
		    if ( ! empty( $item->attr_title ) ) {
				...
				$item_output .= '<a class="pea-top-nav-link"' . $attributes . '>';
			}else{
				$item_output .= '<a class="pea-sub-link"' . $attributes . '>';
			}
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( $container ) {
				$fb_output = '<' . tag_escape( $container );
			...
			$fb_output .= '<li class="menu-item">
				<a class="pea-top-nav-link add-menu" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">'
				. esc_html__( 'Add a menu', 'unlimited-elementor-inner-sections-by-boomdevs' ) .
				'</a>
			</li>';
